            ...
            <div class="card p-0">
                <div class="card-header search-card">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        @can('revenue_create')
                            <a href="{{ route('admin.generalSetting.revenue.create') }}"
                            ...
                            <tbody>
                                @forelse($revenues as $key=>$revenue)
                                    <tr>
                                        ...
                                        <td>{{ $revenue->title }}</td>
                                        <td>{{ get_nepali_number($revenue?->code_no ?? '') }}</td>
                                        <td>{{ $revenue->revenueCategory->title ?? '' }}</td>
                                        <td>रु. {{ get_nepali_number($revenue->amount ?? 0) }}</td>
                                        <td>
                                            @if ($revenue->is_active == 1)
                                                <a href="{{ route('admin.generalSetting.revenue.update-status', $revenue) }}"
                                        ...
                                        <td class="d-flex gap-1">
                                            @can('revenue_edit')
                                                <a data-bs-type="edit"
                                                    href="{{ route('admin.generalSetting.revenue.edit', $revenue) }}"
                                            ...
                                            @endcan
                                            @can('revenue_delete')
                                                <form action="{{ route('admin.generalSetting.revenue.destroy', $revenue) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('delete')
